...
void splitSignal_SMaTGC(TString inputFile){
  ...
  */
  ...
  for (int i=0;i<list_histos->GetEntries();i++){
	...
	for (int j=1;j<=N_bins;j++){
	  cout << "corr: "<<histo_corr->GetBinContent(j)<<" +- "<< histo_corr->GetBinError(j)<<" -> set yield: "<<plot_aTGC_up->GetBinContent(j)<<" -> "<<plot_aTGC_up->GetBinContent(j)*(1.+histo_corr->GetBinError(j)/histo_corr->GetBinContent(j)) << endl;
	...
	TH1D * plot_aTGC_down=(TH1D*)(plot_aTGC->Clone("name_histo_atgcX"));
	for (int j=1;j<=N_bins;j++){
	  cout << "corr: "<<histo_corr->GetBinContent(j)<<" +- "<< histo_corr->GetBinError(j)<<" -> set yield: "<<plot_aTGC_down->GetBinContent(j)<<" -> "<<plot_aTGC_down->GetBinContent(j)*(1.+histo_corr->GetBinError(j)/histo_corr->GetBinContent(j)) << endl;
...
//      cout <<"\t\tbin yield starting: "<< plot->Integral()<<" after rebinning: "<<plot_rebin->Integral()  << endl;
  ...
  for (int i=0;i<list_histos_signal->GetEntries();i++){
    ...
    cout<<"original: "<<plot_signal->GetBinContent(60,60)<<endl;
