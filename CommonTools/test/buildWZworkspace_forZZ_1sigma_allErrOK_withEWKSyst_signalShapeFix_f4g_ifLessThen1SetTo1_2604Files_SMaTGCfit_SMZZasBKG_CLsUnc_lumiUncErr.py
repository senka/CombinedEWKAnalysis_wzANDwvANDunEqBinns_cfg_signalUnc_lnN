...
#    print '\t ----> isItCorrelated: testing ',name
    if ('_eff_b' in name or '_les' in name or '_pu' in name or '_umet' in name or '_res_j' in name or '_scale_j' in name or '_ewk' in name or '_QCD_ACC_JVeto' in name or 'CMS_hzz2l2v_stat_ll_zz2l2nu_8TeV' in name):
#        print '-> true'
        return True
    else:
#        print '-> false'
...
for section in fit_sections:
    ...
    print '****************************************************** using file: %s/%s_boosted.root **************'%(basepath,codename)
...
#            print ' signal shape syst: ',signal_shapeSyst[i]
    ...
    wpt.setBinning(bins)
    lz = theWS.factory('lZ[0., -0.02, 0.02]')
    lz.setConstant(False)
    dkg = theWS.factory('dkg[0.,-0.02, 0.02]')
    dg1 = theWS.factory('dg1[0.,-0.02,0.02]')
...
#                print ' name_forCorr= ',name_forCorr
                ...
                name_forCorr=name_forCorr.replace('zz2l2nu_CMS','CMS')
...
#            print ' \n\t\t ==================================> SIGNAL  zz2l2nu_CMS_scale_j <=========================== ',isItCorrelated('atgc_signal_CMS_scale_j')
            #change name here
...
#                print ' name_forCorr= ',name_forCorr
                name_forCorr=name_forCorr.replace('atgc_signal_CMS','CMS')
                name_forCorr=name_forCorr.replace('zz2l2nu_CMS','CMS')
#                print ' -> name_forCorr= ',name_forCorr
...
#            theta[i] = theWS.var("%s"%signal_shapeSyst[i])
    ...
    aTGCPdf = RooATGCSemiAnalyticPdf_wz('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s'%codename,
                                        ...
                                        dg1,
                                        dibosonPdf,
                                        '%s/signal_WV_%s_f4g_ifLessThen1SetTo1_2604Files_SMaTGCfit.root'%(basepath,codename),
                                        limtype
                                        )

    print '****************************************************** using file: %s/signal_WV_%s_f4g_ifLessThen1SetTo1_2604Files_SMaTGCfit.root**************'%(basepath,codename)
...
#            getattr(theWS, 'import')(aTGCPdf_norm[i])
    ...
    print '****************************************************** using file: %s_boosted_ws.root **************'%(codename)
...
CMS_eff_{codename[0]}           lnN     1.03          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    ...
    for i in range(0,Nbkg_int):
        if (i==2 or i==3): card += """\t\t\t\t1.03""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
...
#QCDscale_VV           lnN     -          """
#    for i in range(0,Nbkg_int):
#        if (i==2 or i==3): card += """\t\t\t\t1.054""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
#        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
...
pdf_VV           lnN     1.058          """
    for i in range(0,Nbkg_int):
        ...
        elif (i==3): card += """\t\t\t\t1.058""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        
    if (doSignalShape_unc):
        for i in range(0,len(signal_shapeSyst)):
            #change name here:
            if (isItCorrelated(str(signal_shapeSyst[i])) and '_ewk' in (str(signal_shapeSyst[i]))):
                name_forCorr=str(signal_shapeSyst[i])
                name_forCorr=name_forCorr.replace('atgc_signal_CMS','CMS')
                name_forCorr=name_forCorr.replace('zz2l2nu_CMS','CMS')
                card += """
{signal_shapeSyst}        shape1  1.0          """.format(signal_shapeSyst=name_forCorr)
            ...
            elif (isItCorrelated(str(signal_shapeSyst[i]))):
                name_forCorr=str(signal_shapeSyst[i])
                name_forCorr=name_forCorr.replace('atgc_signal_CMS','CMS')
                name_forCorr=name_forCorr.replace('zz2l2nu_CMS','CMS')
                card += """
