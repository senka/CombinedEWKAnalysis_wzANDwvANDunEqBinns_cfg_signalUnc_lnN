...
for cfg_item in cfg_items:
    if 'model' in cfg_item:
        planeID = cfg.get('Global','model')     

print 'modelName= ',planeID
...
#    planeID = 'dkglZ'
    ...
    if ( planeID == 'dkglZ' or planeID == '0'):
        limtype = 0
    elif ( planeID == 'dkzdg1zToCW'):
        limtype = 1
    elif ( planeID == 'dkzdg1zToCB'):
        limtype = 2
    elif ( planeID == 'dg1zlZToCWWW'):
        limtype = 3
#    elif ( planeID == 'dg1lZ' ):
#        limtype = 2
#    elif ( planeID == 'dkgdg1'):
#        limtype = 3
        ...
        raise RuntimeError('InvalidCouplingChoice %s'%planeID,
                           'We can only use [dkg,lZ], [dg1,lZ], and [dkg,dg1]'\
                           ' as POIs right now!')
    print '$$$$$$$$$$$$$ \t $$$$$$$$$  setting up for %s plane!'%planeID,' limitType: ', limtype
    ...
    print ' buildWorkspace SemiAnalyticPdf 1...'

    aTGCPdf = RooATGCSemiAnalyticPdf_wz_2Dpol2('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s'%codename,
                                    ...
                                    #    dg1,
                                        dibosonPdf,
 #                                       '%s/signal_WV_%s_2Dpol2.txt'%(basepath,codename),
    ...
    if (doSignalShape_unc):
        ...
        for i in range(0,len(signal_shapeSyst)):
            ...
            print ' buildWorkspace SemiAnalyticPdf 2...'
            aTGCPdf_up[i] = RooATGCSemiAnalyticPdf_wz_2Dpol2('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s_%sUp'%(codename,name_forCorr),
                                                   'ATGCPdf_WV_%s'%codename,
                                                   wpt,
                                                   dkg,
                                                   lz,                                 
...
#                                                   '%s/signal_WV_%s_2Dpol2.txt'%(basepath,codename),
            ...
            print ' ...2'
            print ' buildWorkspace SemiAnalyticPdf 3...'
            aTGCPdf_down[i] = RooATGCSemiAnalyticPdf_wz_2Dpol2('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s_%sDown'%(codename,name_forCorr),
                                                     'ATGCPdf_WV_%s'%codename,
                                                     wpt,
                                                     dkg,
                                                     lz,                                 
